...
namespace {
const Real rho_unit = 4.38365940580630e-10;
const Real cs_unit = 1093174.4074857817;
const Real mu = 0.5984149240266711; //0.60276703256504693;
}
...
Real pressureEq(Real temp, Real rho) {
  ...
  return ((1/3.)*a*std::pow(temp,4.0))+rho*kB*temp/(mu*mProton);
    //return ((1/3.)*a*std::pow(temp,4.0))+rho*8.314462618e7*temp/mu;
}
...
Real energyEq(Real temp, Real rho) {
  ...
  //  return (a*std::pow(temp,4.0))+1.5*rho*8.314462618e7*temp/mu;
}
