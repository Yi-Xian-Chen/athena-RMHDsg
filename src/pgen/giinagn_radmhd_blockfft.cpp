...
//Old version: radiation/radiation.hpp, 
//use prad-> instead of prad->
//also if Radiation-enabled
...
// define once (outside loops, ideally per MeshBlock)
static std::mt19937 gen(12345);
static std::uniform_real_distribution<Real> dist(-0.01, 0.01);
...
void Mesh::InitUserMeshData(ParameterInput *pin) { 
  ...
  tfloor = pin->GetReal("hydro", "pfloor")/rhofloor;
  
  AllocateUserHistoryOutput(8);
  EnrollUserHistoryOutput(0, HistoryBxBy, "-BxBy");
  EnrollUserHistoryOutput(1, HistoryReynolds, "vxvy"); //integrated gravitational stress, 
  EnrollUserHistoryOutput(2, HistoryGrav, "gxgy");  //integrated Reynolds stress, clear
  EnrollUserHistoryOutput(3, Historypressure, "press"); //integrated pressure, clear
  EnrollUserHistoryOutput(4, HistorymidQ, "midrho");
  EnrollUserHistoryOutput(5, HistoryQT, "QT");
  EnrollUserHistoryOutput(6, HistorydVx2, "vx2"); //integrated 0.5*rho vx^2, check if its same as 1KE output by history.cpp
  EnrollUserHistoryOutput(7, HistorydVy2, "vy2"); // integrated 0.5*rho (delta vy)^2, will be clear if vx2 is good
...
if(mesh_bcs[BoundaryFace::outer_x3] == GetBoundaryFlag("user")) {
  ...
  EnrollUserRadBoundaryFunction(BoundaryFace::outer_x3, Outflow_rad_X2);
        ...
        fluxtable.NewAthenaArray(256);
            ...
            if ( (fflux=fopen("./F_prelim.txt","r"))==NULL )
            {
              printf("Open input file error");
              return;
            }
  ...
  Real rideal = 8.314462618e7/0.62;
  Real arad = 7.5657332500e-15;
  ...
  if(RADIATION_ENABLED||IM_RADIATION_ENABLED){
   ...
   rhounit = pin->GetOrAddReal("radiation", "density_unit", 1.e-8); //Tcgs
   tunit = pin->GetOrAddReal("radiation", "T_unit", 1.0);//rhocgs
   lunit = pin->GetOrAddReal("radiation", "length_unit", 1.496e12);  // cs(rho, T)/Omega
  }
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  Real kxb = (2.0*PI/Lx);// nxw=-ve for leading wave
    // Compute field strength based on beta.
  if (MAGNETIC_FIELDS_ENABLED) {
    beta = pin->GetReal("problem","beta"); //thermal beta. total beta is beta/(prat/3+1)
    B0 = std::sqrt(static_cast<Real>(2.0/beta));
  ...
  for (int k=ks; k<=ke; k++) {
        ...
        if (strat){
          ...
          //den = densitytable(z_id);
            if (z_id <= 0) {
                ...
                else if (z_id >= 256) {   // Nz = total size of table
                    den = densitytable(256 - 1);
                    radflx = fluxtable(256 - 1);
                } 
                else {
                    Real z0 = ztable(z_id - 1) * std::sqrt(pratmidplane/3 + 1);
                    Real z1 = ztable(z_id)     * std::sqrt(pratmidplane/3 + 1);

                    Real rho0 = densitytable(z_id - 1);
                    Real flux0 = fluxtable(z_id - 1);
                    Real rho1 = densitytable(z_id);
                    Real flux1 = fluxtable(z_id);

                    Real frac = (x3 - z0) / (z1 - z0);

                    den = rho0 + frac * (rho1 - rho0);
                    radflx = flux0 + frac * (flux1 - flux0);
        ...
        rd = amp*den*std::cos(kx*x1 + ky*x2); //stratified disk is without initial perturbations
        
        Real temp = std::max(std::pow(den, 1.0/3.0), tfloor);
        ...
        // inside your loop
        Real dvx = dist(gen); //0.0
        Real dvy = dist(gen);
        Real dvz = dist(gen);
              
        
        phydro->u(IM1,k,j,i) = (den)*rvx + den*dvx;
        phydro->u(IM2,k,j,i) = (den)*rvy + den*dvy;
        if (pmy_mesh->shear_periodic) {
          phydro->u(IM2,k,j,i) -= (den)*(qshear*Omega0*x1);
        }
        ...
        if (MAGNETIC_FIELDS_ENABLED) {
            ...
            // Net vertical flux:
            // Bx = By = 0
            // Bz = B0 * (1 + 0.5 sin(kx x))
            // where kx = 2π / Lx (already defined)

            Real Bz = B0 * (1.0 + 0.5 * std::sin(kxb * x1));

            pfield->b.x1f(k,j,i) = 0.0;
            pfield->b.x2f(k,j,i) = 0.0;
            pfield->b.x3f(k,j,i) = Bz;

            // Set upper faces (Athena++ face-centered fields)
            if (i == ie) pfield->b.x1f(k,j,ie+1) = 0.0;
            if (j == je) pfield->b.x2f(k,je+1,i) = 0.0;
            if (k == ke) pfield->b.x3f(ke+1,j,i) = Bz;
          }
        ...
        if(RADIATION_ENABLED || IM_RADIATION_ENABLED){
          ...
          Real er = std::max(gast * gast * gast * gast, 1.e-8); //set some floor value initially   std::abs(radflx)
            radflx = 0.0; //override for simplified case
...
{
    int znum = 0;
    while (x3 > ztable(znum) * std::sqrt(pratmidplane/3 + 1)) {
        znum++;
    }
    return znum;
}
    ...
    for (int k=1; k<=ngh; k++) {
      for (int j=js; j<=je; j++) {
        for (int i=is; i<=ie+1; i++) {
          b.x1f(ke+k,j,i) = 0.0;//b.x1f(ke,j,i);
        }
      }
    }
    for (int k=1; k<=ngh; k++) {
      for (int j=js; j<=je+1; j++) {
        for (int i=is; i<=ie; i++) {
          b.x2f(ke+k,j,i) = 0.0;//b.x2f(ke,j,i);
        }
      }
    }
    for (int k=1; k<=ngh; k++) {
      ...
      Real time, Real dt, int is, int ie, int js, int je, int ks, int ke, int ngh)
{
  ...
  if (MAGNETIC_FIELDS_ENABLED) {
    for (int k=1; k<=ngh; k++) {
      for (int j=js; j<=je; j++) {
        for (int i=is; i<=ie+1; i++) {
          b.x1f(ks-k,j,i) = 0.0; //b.x1f(ks,j,i);
        }
      }
    }
    for (int k=1; k<=ngh; k++) {
      for (int j=js; j<=je+1; j++) {
        for (int i=is; i<=ie; i++) {
          b.x2f(ks-k,j,i) = 0.0; //b.x2f(ks,j,i);
        }
      }
    }
    for (int k=1; k<=ngh; k++) {
...
void DiskOpacity(MeshBlock *pmb, AthenaArray<Real> &prim)
{
  ...
  for (int ifr=0; ifr<prad->nfreq; ++ifr){
    ...
    if(kappa < kappas){
      if(gast < 10.0){
        kappaa = kappa;//make absorption opacity =0.34
        kappa = 0.0;
      }else{
        kappaa = 0.0; //make everything scattering
      }
    }else{
      kappaa = kappa - kappas;//usually make absorp = rossmean - scattering
      kappa = kappas; //assign scatter opacity
    }
     //overwrite
    kappa = kappas;
    kappaa = 0.05; //std::max(0.05, kappaa);
    //Real factor = 1.0;
    //prad->sigma_s(k,j,i,ifr) = kappa * rho * rhounit * lunit * factor;
    //prad->sigma_a(k,j,i,ifr) = kappaa * rho * rhounit * lunit* factor;
    //prad->sigma_p(k,j,i,ifr) = kappaa * rho * rhounit * lunit* factor; //kappa_planck * rho * rhounit * lunit* factor;
    ...
    prad->sigma_s(k,j,i,ifr) = kappa * std::min(rho*rho*rho/1e-6, rho) * rhounit * lunit;
    prad->sigma_a(k,j,i,ifr) = kappaa * std::min(rho*rho*rho/1e-6, rho) * rhounit * lunit;
    prad->sigma_ae(k,j,i,ifr) = prad->sigma_a(k,j,i,ifr);
    if(kappaa < kappa_planck)
      prad->sigma_planck(k,j,i,ifr) = 0.0; //(kappa_planck-kappaa)*rho*rhounit*lunit;
    else
