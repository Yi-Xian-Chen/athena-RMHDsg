...
void Mesh::InitUserMeshData(ParameterInput *pin) { 
  ...
  EnrollUserHistoryOutput(0, HistoryReynolds, "vxvy");
  EnrollUserHistoryOutput(1, HistoryGrav, "gxgy");
  EnrollUserHistoryOutput(2, Historypressure, "press");
  EnrollUserHistoryOutput(3, HistorymidQ, "midQ");
  EnrollUserHistoryOutput(4, HistoryQT, "QT");
  EnrollUserHistoryOutput(5, HistorydVx2, "vx2");
  EnrollUserHistoryOutput(6, HistorydVy2, "vy2");
...
*/
  ...
  if (NON_BAROTROPIC_EOS) {
    p0 = SQR(cs)*d0; // midplane pressure
  }
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
    ...
    std::cout << "sigma0 = " << d0*scaleH*4*std::sqrt(2)*SQR(I3Q) << std::endl;
  ...
  for (int k=ks; k<=ke; k++) {
    ...
    if (strat) { //if stratified
        ...
        if (strat){
          ...
          rd = amp*std::cos(kx*x1 + ky*x2); //stratified disk is with initial perturbations
        }
        else {
          den = d0;
...
// a more complicated setup
                        ...
                        if (std::abs(radflx) > er)
                        {
                          radflx = -2.04063e-11; //need to tune this to be equal to the Erad constant
                          er = 2.04063e-11;
                        }
...
//Reynolds stress, not density averaged
Real HistoryReynolds(MeshBlock *pmb, int iout) {
...
//Gravitational stress, not density averaged
Real HistoryGrav(MeshBlock *pmb, int iout) {
 Real drhogxgy = 0.0;
 int is = pmb->is, ie = pmb->ie, js = pmb->js, je = pmb->je, ks = pmb->ks, ke = pmb->ke;
 AthenaArray<Real> &w = pmb->phydro->w;
 ...
 for (int k=ks; k<=ke; k++) {
  for (int j=js; j<=je; j++) {
   ...
   for (int i=is; i<=ie; i++) {
     ...
     drhogxgy += volume(i)*gx*gy/4/PI/gconst;
   }
  }
 }
  Real Grav = drhogxgy;
...
//averaged pressure, not density weighted
...
Real Historypressure(MeshBlock *pmb, int iout) {
 ...
 // namespace
    ...
    //averaged Qmid, not density weighted
Real HistorymidQ(MeshBlock *pmb, int iout) {
 Real Qmid = 0.0;
 Real vol = 0.0;
 int is = pmb->is, ie = pmb->ie, js = pmb->js, je = pmb->je, ks = pmb->ks, ke = pmb->ke;
 ...
 for (int k=ks; k<=ke; k++) {
  ...
  for (int j=js; j<=je; j++) {
   pmb->pcoord->CellVolume(k,j,pmb->is,pmb->ie,volume);
   for (int i=is; i<=ie; i++) {
     Qmid += volume(i)/w(IDN,k,j,i)/2/PI;
     vol += volume(i);
   }
  }
 ...
 return Qmid/vol; //note I have not divided with G
}
 
    //averaged QT, not density weighted
...
Real HistoryQT(MeshBlock *pmb, int iout) {
 ...
 for (int k=ks; k<=ke; k++) {
  for (int j=js; j<=je; j++) {
   pmb->pcoord->CellVolume(k,j,pmb->is,pmb->ie,volume);
   for (int i=is; i<=ie; i++) {
     QT += volume(i)*std::sqrt(w(IPR,j,k,i)/w(IDN,j,k,i))/PI/gconst;
     vol += volume(i);
   }
  }
 }
 return QT/vol; //note I have not divided with G
}
...
Real HistorydVx2(MeshBlock *pmb, int iout) {
  ...
  for (int k=ks; k<=ke; k++) {
    for (int j=js; j<=je; j++) {
      pmb->pcoord->CellVolume(k,j,pmb->is,pmb->ie,volume);
      for (int i=is; i<=ie; i++) {
        dvx2 += 0.5*volume(i)*w(IDN,k,j,i)*w(IVX,k,j,i)*(w(IVX,k,j,i));
        vol += volume(i);
      }
    }
  }
  ...
  for (int k=ks; k<=ke; k++) {
    for (int j=js; j<=je; j++) {
      ...
      for (int i=is; i<=ie; i++) {
        ...
        dvy2 += 0.5*volume(i)*w(IDN,k,j,i)*(w(IVY,k,j,i) - vshear)*(w(IVY,k,j,i) - vshear);
        vol += volume(i);
      }
    }
  }
...
//gas internal energy, not density averaged
...
Real HistoryEtot(MeshBlock *pmb, int iout) {
  ...
  for (int k=ks; k<=ke; k++) {
    for (int j=js; j<=je; j++) {
      pmb->pcoord->CellVolume(k,j,pmb->is,pmb->ie,volume);
      for (int i=is; i<=ie; i++) {
        dE += volume(i)*w(IEN,k,j,i)/gamma1;
        vol += volume(i);
      }
    }
  }
...
/*
Real HistoryFtot(MeshBlock *pmb, int iout) {
  Real dE = 0.0;
  Real vol = 0.0;
  int is = pmb->is, ie = pmb->ie, js = pmb->js, je = pmb->je, ks = pmb->ks, ke = pmb->ke;
  AthenaArray<Real> &w = pmb->phydro->w;
