...
void coolingrate2(const Real rho, const Real press, Real &edot);
  ...
  for (int k = pmb->ks; k <= pmb->ke; ++k) {
    for (int j = pmb->js; j <= pmb->je; ++j) {
      for (int i = pmb->is; i <= pmb->ie; ++i) {
        ...
        //coolingr = std::min(coolingr, 1.0); // Ensure coolingr is at most 1.0
      cons(IEN,k,j,i) -=  std::min(prim(IDN,k,j,i) * coolingr, prim(IEN,k,j,i)) * dt; //This can be tuned
      //cons(IEN,k,j,i) -= prim(IDN,k,j,i) * coolingr* dt ; //This can be tuned
	}
    }
  }
  return;
...
void Mesh::InitUserMeshData(ParameterInput *pin) { 
  ...
  EnrollUserHistoryOutput(7, HistoryCooling, "coolingF");
  ...
  EnrollUserBoundaryFunction(BoundaryFace::inner_x3, Inflow_X1);
        ...
        coolingtable.NewAthenaArray(50,100);//example
        logPtable.NewAthenaArray(50);
        logrhotable.NewAthenaArray(100);
            ...
            fcooling = fopen("./cooling_new.txt", "r");
            flogP = fopen("./logPtorho.txt", "r");
...
void coolingrate2(const Real rho, const Real press, Real &edot) // Interpolate cooling rate from table
{
    ...
    Real logP_minus_rho = logP - logrho;

    // Clamp logrho to within bounds
    if (logrho < logrhotable(0)) logrho = logrhotable(0);
    if (logrho > logrhotable(99)) logrho = logrhotable(99);

    // Clamp logP - logrho to within bounds
    if (logP_minus_rho < logPtable(0)) logP_minus_rho = logPtable(0);
    if (logP_minus_rho > logPtable(49)) logP_minus_rho = logPtable(49);

    // Locate bracketing indices for logrho
    int rho_idx1 = 0, rho_idx2 = 0;
    while ((logrho > logrhotable(rho_idx2)) && (rho_idx2 < 99)) {
        rho_idx1 = rho_idx2;
        rho_idx2++;
    }
    if (logrho == logrhotable(99)) {
        rho_idx1 = 98;
        rho_idx2 = 99;
    }

    // Locate bracketing indices for logP - logrho
    int P_idx1 = 0, P_idx2 = 0;
    while ((logP_minus_rho > logPtable(P_idx2)) && (P_idx2 < 49)) {
        P_idx1 = P_idx2;
        P_idx2++;
    }
    if (logP_minus_rho == logPtable(49)) {
        P_idx1 = 48;
        P_idx2 = 49;
    }

    // Get surrounding cooling values from the table
    Real cool_P1_rho1 = coolingtable(P_idx1, rho_idx1);
    Real cool_P1_rho2 = coolingtable(P_idx1, rho_idx2);
    Real cool_P2_rho1 = coolingtable(P_idx2, rho_idx1);
    Real cool_P2_rho2 = coolingtable(P_idx2, rho_idx2);

    Real rho_1 = logrhotable(rho_idx1);
    Real rho_2 = logrhotable(rho_idx2);
    Real P_1 = logPtable(P_idx1);
    Real P_2 = logPtable(P_idx2);

    // Perform bilinear interpolation
    if (rho_idx1 == rho_idx2) {
        if (P_idx1 == P_idx2) {
            edot = cool_P1_rho1;
        } else {
            edot = cool_P1_rho1 + (cool_P2_rho1 - cool_P1_rho1) *
                                   (logP_minus_rho - P_1) / (P_2 - P_1);
        }
    } else {
        if (P_idx1 == P_idx2) {
            ...
            edot = cool_P1_rho1 * (P_2 - logP_minus_rho) * (rho_2 - logrho) /
                                   ((P_2 - P_1) * (rho_2 - rho_1)) +
                   cool_P2_rho1 * (logP_minus_rho - P_1) * (rho_2 - logrho) /
                                   ((P_2 - P_1) * (rho_2 - rho_1)) +
                   cool_P1_rho2 * (P_2 - logP_minus_rho) * (logrho - rho_1) /
                                   ((P_2 - P_1) * (rho_2 - rho_1)) +
                   cool_P2_rho2 * (logP_minus_rho - P_1) * (logrho - rho_1) /
                                   ((P_2 - P_1) * (rho_2 - rho_1));
        }
    }

    edot = edot * rhocgs / Pcgs / Omegacgs;
}
...
void coolingrate(const Real rho, const Real press, Real &edot) //interpolate cooling rate from table
{
    ...
    // Early return if input is outside the bounds of the tables
    if (logrho < logrhotable(0) || logrho > logrhotable(99) ||
        (logP - logrho) < logPtable(0) || (logP - logrho) > logPtable(49)) {
        edot = 1e11 * rhocgs / Pcgs / Omegacgs;
        return;
    }
    ...
    int P_idx1 = 0;
    int P_idx2 = 0;
    while ((logP - logrho > logPtable(P_idx2)) && (P_idx2 < 49)) {
        P_idx1 = P_idx2;
        P_idx2++;
    }
    if (P_idx2 == 49 && (logP - logrho > logPtable(P_idx2)))
        P_idx1 = P_idx2;
